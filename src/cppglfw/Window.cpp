...
void Window::setSizeLimits(const int32_t min_width, const int32_t min_height, const int32_t max_width,
                           const int32_t max_height) const {
  if (data_->destroyed) {
    throw std::runtime_error("Tried to use destroyed window.");
  }
...
void Window::setOnPositionChangedCallback(const std::string& id,
...
void Window::setOnResizeCallback(const std::string& id,
                                 const std::function<void(Window, int32_t, int32_t)>& callback) const {
...
void Window::setOnCursorEnterCallback(const std::string& id, const std::function<void(Window, bool)>& callback) const {
...
Window::WindowData::WindowData(GLFWwindow* window) : window(window), destroyed(window == nullptr) {}
